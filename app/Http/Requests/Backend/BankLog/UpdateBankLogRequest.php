<?php

namespace App\Http\Requests\Backend\BankLog;

use App\Models\BankLog\BankLog;
use Illuminate\Auth\Access\AuthorizationException;
use Illuminate\Foundation\Http\FormRequest;

/**
 * Class UpdateBankLogRequest.
 */
class UpdateBankLogRequest extends FormRequest
{
    /**
     * Determine if the bank is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            'content_fix' => ['required'],
            'note' => ['sometimes'],
            'name' => ['sometimes'],
            'fund_category_id' => ['sometimes'],
            'benAccountNo' => ['sometimes'],
        ];
    }

    /**
     * @return array
     */
    public function messages()
    {
        return [
        ];
    }

    /**
     * Handle a failed authorization attempt.
     *
     * @return void
     *
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    protected function failedAuthorization()
    {
        throw new AuthorizationException(__('Only the administrator can update this bank.'));
    }
}
